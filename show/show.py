...
def show_stp_vlan(ctx, vlanid):
    ...
    if ctx.invoked_subcommand is None:
        ...
        eth_list = [int(ifname[len("Ethernet"):]) for ifname in intf_list if ifname.startswith("Ethernet")]
        po_list = [int(ifname[len("PortChannel"):]) for ifname in intf_list if ifname.startswith("PortChannel")]
