...
def stp_clr_stats_intf(ctx, interface_name):
    command = ['sudo', 'docker', 'exec', 'stp', 'stpctl', 'clrstsintf', interface_name]
    clicommon.run_command(command)
...
def stp_clr_stats_vlan(ctx, vlan_id):
    command = ['sudo', 'docker', 'exec', 'stp', 'stpctl', 'clrstsvlan', vlan_id]
    clicommon.run_command(command)
...
def stp_clr_stats_vlan_intf(ctx, vlan_id, interface_name):
    command = ['sudo', 'docker', 'exec', 'stp', 'stpctl', 'clrstsvlanintf', vlan_id, interface_name]
    clicommon.run_command(command)
